...
def input_attention(entity1,entity2,sentence,word_embed,position_embed,position1,position2):
    """
    构建input_attention
    ...
    :return:
    """
    def _get_a(entity_embed,sentence):
        A_j =tf.expand_dims(entity_embed,axis=-1)
        a_j = tf.matmul(sentence,A_j)
        a_j = tf.nn.softmax(tf.reshape(a_j,[-1,FLAGS.max_len]))
        return a_j
    ...
    entity1_embed = tf.nn.embedding_lookup(word_embed, entity1)
    entity2_embed = tf.nn.embedding_lookup(word_embed, entity2)
    nposition1 = tf.nn.embedding_lookup(position_embed, position1)
    nposition2 = tf.nn.embedding_lookup(position_embed, position2)
    z = _get_z(nposition1, nposition2, sentences)
    # a_1 = _get_a(entity1_embed,sentences)
    # a_2 = _get_a(entity2_embed,sentences)
    # input = tf.multiply(z,tf.reshape(tf.add(a_1,a_2)/2,[-1,FLAGS.max_len,1]))
    return z

def attention_pooling(conv_output,label_embed,num_filter,numclass,is_train=True):
    ...
    if is_train:
        max_pool = tf.nn.dropout(max_pool,FLAGS.keep_prob)
    return max_pool

def cnn_layer(name, input_data, num_filter,label_embed, numclass,is_train):
    ...
    with tf.variable_scope(name):
        ...
        with tf.variable_scope("conv-1"):
            ...
            max_pool = attention_pooling(R_tanh,label_embed,num_filter,numclass,is_train)
...
class Model(object):
    def __init__(self, word_embed, data, num_classes,num_filter, position_dim, is_train=True,regulizer = True):
        ...
        word_embeds = tf.get_variable("word_embed", initializer=word_embed,
        ...
        input_data = input_attention(entity1, entity2, sentence, word_embeds, possition_embed, position1, position2)
        w_0 = cnn_layer("feature", input_data, num_filter,label_embed,num_classes,is_train)
    ...
    def predict_distance(self,matrix1,matrix2):
        ...
        all_distance = tf.norm(wo - tf.nn.l2_normalize(matrix2,1), axis=2,ord=2)
        return all_distance
    ...
    def loss_distance(self,matrix1,matrix2):
        ...
        distance = tf.norm(disance,ord=2,axis=1)
        return distance

    def loss(self,w_0, label_embed,real_label,label):
        distance = self.predict_distance(w_0, label_embed)
        mask = tf.one_hot(label,FLAGS.num_classes,on_value=1000.,off_value=0.)
...
def train_or_valid(train_data, test_data, word_embed):
    with tf.name_scope("Train"):
        with tf.variable_scope("model", reuse=None):
            mtrain = Model(word_embed,  train_data, FLAGS.num_classes,FLAGS.num_filter,
                           position_dim=FLAGS.position_dim,
                           is_train=True)

    with tf.name_scope("test"):
        with tf.variable_scope("model", reuse=True):
            mtest = Model(word_embed,  test_data, FLAGS.num_classes,
